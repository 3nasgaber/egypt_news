...
import 'package:egypt_news/services/webservice.dart';
...
class NewsArticleListViewModel extends ChangeNotifier {
  ...
  Future<void> search(String keyword) async {
    ...
    notifyListeners();
    List<ArticlesModel> newsArticles = await Webservice().fetchHeadlinesByKeyword(keyword);
  ...
  Future<void> populateTopHeadlines() async {
    this.loadingStatus = LoadingStatus.searching;
    notifyListeners();
    List<ArticlesModel> newsArticles = await Webservice().fetchTopHeadlines();
    this.articles = newsArticles.map((article) => NewsArticleViewModel(article: article)).toList();
    this.loadingStatus = this.articles.isEmpty ? LoadingStatus.empty : LoadingStatus.completed;
    notifyListeners();
  }
